...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zvitTracker = unique_ptr<CzAureusXIVTracker>(new CzAureusXIVTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZVIT_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZVIT_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
