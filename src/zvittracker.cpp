...
CzAureusXIVTracker::CzAureusXIVTracker(std::string strWalletFile)
{
...
CzAureusXIVTracker::~CzAureusXIVTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAureusXIVTracker::Init()
{
...
bool CzAureusXIVTracker::Archive(CMintMeta& meta)
{
...
bool CzAureusXIVTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAureusXIVTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAureusXIVTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAureusXIVTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzAureusXIVTracker::GetSerialHashes()
{
...
CAmount CzAureusXIVTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAureusXIVTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAureusXIVTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAureusXIVTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAureusXIVTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAureusXIVTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAureusXIVTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAureusXIVTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAureusXIVTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAureusXIVTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAureusXIVTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAureusXIVTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAureusXIVTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAureusXIVTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAureusXIVTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAureusXIVTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAureusXIVTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzvit from DB\n", __func__, listDeterministicDB.size());
    }
